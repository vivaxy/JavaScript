  ...
  function parseExpressionStatement(startIndex, endIndex) {
    ...
    while (i < endIndex) {
      ...
      if (code === codes['(']) {
        ...
        const expr = parseExpressionStatement(_startIndex + 1, _endIndex)
          .expression;
        expr.paren = true;
        consumeNode(expr);
        i = _endIndex + 1;
      ...
      if (code === codes['?']) {
        ...
        consumeConditionalExpression(
          new astFactory.ConditionalExpression(
            ...
            parseExpressionStatement(colonIndex + 1, input.length).expression
          )
        );
        i = input.length;
        break;
    ...
    function consumeNode(node) {
      ...
      if (operator === null && secondOperator === null && node.paren === true) {
        delete node.paren;
        return consumeCallExpression(node);
      }
