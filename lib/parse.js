  ...
  function parseExpressionStatement(input, depth) {
    ...
    while (i < input.length) {
      ...
      if (matchToken('undefined')) {
        i += 9;
        consumeNode(new astFactory.Literal(undefined));
        continue;
      }
      ...
      if (code === codes['(']) {
        ...
        const expr = parseExpressionStatement(
      ...
      if (code === codes['[']) {
        ...
        if (operator || secondOperator) {
          consumeArrayExpression(input.slice(startIndex + 1, endIndex));
        } else {
          consumeMemberExpression(
    ...
    function consumeArrayExpression(input) {
      const expr = new astFactory.ArrayExpression(
        input.split(',').map((inputSection) => {
          return parseExpressionStatement(inputSection, 0).expression;
        })
      );
      consumeNode(expr);
    }
