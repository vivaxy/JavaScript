...
const serializeTypes = {
  ...
  [astTypes.BINARY_EXPRESSION](ast, parent) {
    ...
    if (
      ...
      (parent.type === astTypes.BINARY_EXPRESSION &&
        (astFactory.binaryOperatorPrecedences[parent.operator] >
  ...
  [astTypes.CONDITIONAL_EXPRESSION](ast, parent) {
    const serialized =
      serialize(ast.test, ast) +
      ' ? ' +
      serialize(ast.consequent, ast) +
      ' : ' +
  ...
  [astTypes.MEMBER_EXPRESSION](ast) {
    if (ast.computed) {
      return (
        serialize(ast.object, ast) + '[' + serialize(ast.property, ast) + ']'
      );
    }
